...
//            UserCache.saveToken(cookie.getValue());
        }
        // Save cookie into persistent store
        SharedPreferences.Editor prefsWriter = cookiePrefs.edit();
        prefsWriter.putString(cookie.getDomain(), TextUtils.join(",", cookies.get(cookie.getDomain()).keySet()));
        prefsWriter.putString(COOKIE_NAME_PREFIX + cookie.getName(), encodeCookie(new SerializableHttpCookieModel(cookie)));
        prefsWriter.apply();
     ...
     */
    protected String encodeCookie(SerializableHttpCookieModel cookie) {
        if (cookie == null)
            return null;
    ...
    protected HttpCookie decodeCookie(String cookieString) {
        ...
        try {
            ...
            cookie = ((SerializableHttpCookieModel) objectInputStream.readObject()).getCookie();
        } catch (IOException e) {
